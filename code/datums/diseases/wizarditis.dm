...
/datum/disease/wizarditis/proc/teleport()
	set background = 1
	var/list/local_turfs = range_turfs(30, affected_mob)

	var/turf/T
	finding_suitable_turf:
		while(1)
			T = pick_n_take(local_turfs)
			if(!T)						return
			if(T.density)				continue
			if(istype(T, /turf/space))	continue
			if(istype(T.loc, /area/space))	continue
			for(var/atom/movable/AM in T)
				if(AM.density)			continue finding_suitable_turf
			break

	affected_mob.say("SCYAR NILA [uppertext(T.loc.name)]!")
	affected_mob.loc = T
	return
