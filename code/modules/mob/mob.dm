...
/mob/proc/show_message(msg, type, alt, alt_type)//Message, type of message (1 or 2), alternative message, alt message type (1 or 2)
	if(!client)
		return
	if(type)
		if(type & 1 && (sdisabilities & BLIND || blinded || stat) )//Vision related
			if(!alt)
				return
			else
				msg = alt
...
/mob/verb/observe()
	...
	if(is_admin)
		reset_view(mob_eye)
		client.adminobs = 1
		if(mob_eye == client.mob)//Our mob so we are not actually observing
			client.adminobs = 0
		return
		...
		if(client && client.holder)
			...
			if(master_controller)
				...
				for(var/datum/subsystem/SS in master_controller.subsystems)
					stat(SS.name, "[round(SS.cost,0.001)]ds\t(CPU:[round(SS.cpu,1)]%)")
			else
...
/mob/proc/update_canmove()
	if(buckled)
		...
		if(buckled.loc != src.loc)//Our thing is not under us so we are not buckled, this will take affect next time we get updated
