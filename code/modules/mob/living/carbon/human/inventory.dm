	...
	if ((item && !( L.Find(place) )))
		...
		switch(place)
			if("syringe")
				...
				//target.attack_log += text("\[[time_stamp()]\] <font color='orange'>Has been injected (attempt) by [source.name] ([source.ckey])</font>")
				//source.attack_log += text("\[[time_stamp()]\] <font color='red'>Attempted to inject [target.name] ([target.ckey])</font>")

			if("pill")
				message = "\red <B>[source] is trying to force [target] to swallow [item]!</B>"
				//target.attack_log += text("\[[time_stamp()]\] <font color='orange'>Force feed attempt with [item] by [source.name] ([source.ckey])</font>")
				//source.attack_log += text("\[[time_stamp()]\] <font color='red'>Attempted to feed [target.name] ([target.ckey]) [item]</font>")

			if("drink")
				message = "\red <B>[source] is trying to force [target] to swallow a gulp of [item]!</B>"
				//target.attack_log += text("\[[time_stamp()]\] <font color='orange'>Force feed attempt with [item] b [source.name] ([source.ckey])</font>")
				//source.attack_log += text("\[[time_stamp()]\] <font color='red'>Attempted to feed [target.name] ([target.ckey]) [item]</font>")

			if("dnainjector")
				message = "\red <B>[source] is trying to inject [target] with the [item]!</B>"
				//target.attack_log += text("\[[time_stamp()]\] <font color='orange'>Has been injected with DNA (attempt) by [source.name] ([source.ckey])</font>")
			//	source.attack_log += text("\[[time_stamp()]\] <font color='red'>Attempted to inject [target.name] ([target.ckey]) with DNA</font>")
			...
			if("eyes")
				if(target.glasses && !target.glasses.can_remove)
			...
			if("ears")
				if(target.ears && !target.ears.can_remove)
			...
			if("uniform")
				for(var/obj/item/I in list(target.l_store, target.r_store))
					if(I.on_found(source))
						return
				if(target.w_uniform && !target.w_uniform.can_remove)
...
!can_remove refers to variable of /obj/item/clothing which either allows or disallows that item to be removed.
	...
	switch(place)	//here we go again...
		...
		if("CPR")
			if (target.cpr_time >= world.time + 30)
				del(src)
			if((target.health >= -99.0 && target.health <= 0) && target.stat != 2)
				target.cpr_time = world.time
				target.deal_damage(-min(target.oxy_damage, 10), OXY)
				...
				source << "\red Repeat at least every 8 seconds."
		if("dnainjector")
