/obj/item/weapon/grab
	...
	suicide_act(mob/user)
		if(affecting.stat == 2)
			viewers(user) << "\red <b>[user] is attacking \him self with the dead body of [affecting.name]! It looks like \he's trying to commit suicide!</b>"
		else
...
/obj/item/weapon/grab/process()
	...
	if((killing == 2 && state == 3))
		if(assailant.loc != kill_loc)
			...
			return
		if(istype(affecting, /mob/living))
			var/mob/living/L = affecting
			L.deal_damage(5, WEAKEN)//This should likely just prevent him from moving rather than stunning him
