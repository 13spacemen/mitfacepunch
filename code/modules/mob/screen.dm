...
/obj/screen/Click(location, control, params)
	...
	switch(name)
		...
		if("Reset Machine")
			usr.unset_machine()
		if("internal")
			if (( !usr.stat && !usr.paralysis && !usr.restrained() ))
				if (usr.internal)
					usr.internal = null
	...
	if(L.buckled && (L.last_special <= world.time) )
		if(iscarbon(L))
			...
			if( C.handcuffed )
				...
				C << "\red You attempt to unbuckle yourself. (This will take around 1 minutes and you need to stand still)"
	...
	//breaking out of handcuffs
		...
		if(CM.handcuffed && CM.canmove && (CM.last_special <= world.time))
			...
			if(isalienadult(CM) || (HULK in usr.mutations))//Don't want to do a lot of logic gating here.
				...
				visible_message("\red <B>[CM] is trying to break the handcuffs!</B>")
				spawn(0)
					if(do_after(CM, 50))
						if(!CM.handcuffed || CM.buckled)
							return
						visible_message("\red <B>[CM] manages to break the handcuffs!</B>")
				...
				var/displaytime = 1 //Minutes to display in the "this will take X minutes."
